...
#' @param delete a logical balue. If TRUE, time stacks where new cells are non-existent in the previous time stack are deleted.
...
trackCellmod <- function(folder, THRESHOLD = 1, delstack = T){
  ...
  for(i in seq_along(zipfiles)[-1]){
    ...
    if(length(has.to.change)>0){
      ...
      # Checks if the original centroid of the cells is found within the cells of the previous time stack            
